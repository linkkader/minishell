...
void	dollar_counter(char *str, t_expand_var *exp_var)
{
	...
	while (str[++i])
	{
		...
		if (str[i] == '$' && (parse_ft_isalnum(str[i + 1]) \
		|| str[i + 1] == '_' || str[i + 1] == '?'))
			exp_var->count++;
...
void	get_variable(t_env_var	*env_var, char **env)
{
	...
	if (!env_var->var)
		return ;
	parse_ft_strlcpy(env_var->var, var, env_var->var_len + 1);
...
void	replace_var_by_val(char *str, t_expand_var *exp_var)
{
	...
	while (str[++i])
	{
		...
		(parse_ft_isalnum(str[i + 1]) || str[i + 1] == '_' || str[i + 1] == '?'))
		{
			parse_ft_strlcpy(exp_var->new_str + j, exp_var->env_var[index].val, \
...
void	expander(char *str, char **env, t_expand_var *exp_var)
{
	...
	while (str[++i])
	{
		...
		|| str[i + 1] == '_' || str[i + 1] == '?'))
		{
			...
			while (ftparse__isalpha(str[++i]) || str[i] == '_')
