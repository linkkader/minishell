	...
	while (cmds)
	{
		...
		while (cmds->tokens->tokens[++i])
		{
			if (!redirect(cmds->tokens->tokens[i]) && ((i && !redirect(cmds->tokens->tokens[i - 1])) || !i))
			{
				...
				{
					cmds->command_name = expand_var(parse_ft_strdup(cmds->tokens->tokens[i]), env, 0);
					cmds->command_path = get_command_path(cmds->command_name, env);
				}
				cmds->command_args = ft_realloc(cmds->command_args, expand_var(parse_ft_strdup(cmds->tokens->tokens[i]), env, 0));
			}
		}
		cmds = cmds->next;
	}
